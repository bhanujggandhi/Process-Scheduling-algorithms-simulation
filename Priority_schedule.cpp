...
priority_queue<PCB *, vector<PCB *>, CompareProcessPriority> Priority_Ready_queue;
priority_queue<PCB *, vector<PCB *>, CompareProcessPriority> Priority_Device_queue;
...
void schedulePriority()
{
    ...
    {
        auto idlep = PriorityRunning_map[0];
        PriorityRunning_map.erase(0);
        Priority_Ready_queue.push(idlep);
        context_switch(idlep, READY);

        auto currprocess = Priority_Ready_queue.top();
        Priority_Ready_queue.pop();
        PriorityRunning_map.insert({currprocess->pid, currprocess});
