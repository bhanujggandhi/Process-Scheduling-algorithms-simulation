...
unordered_map<int, PCB *> Sjf_PRunning_map;
priority_queue<PCB *, vector<PCB *>, CompareProcessSjf_P> Sjf_P_Ready_queue;
priority_queue<PCB *, vector<PCB *>, CompareProcessSjf_P> Sjf_P_Device_queue;

/* Utility Functions */
vector<PCB *> Sjf_P(){
    vector<PCB *>  Sjf_PVect;
...
void scheduleSjf_P()
{
    ...
    if (Sjf_PRunning_map.size() == 1 and Sjf_PRunning_map.find(0) == Sjf_PRunning_map.end())
    {
        return;
    }
    // If only idle process is present and some other process comes
    if (Sjf_P_Ready_queue.size() >= 1 and Sjf_PRunning_map.size() == 1 and Sjf_PRunning_map.find(0) != Sjf_PRunning_map.end())
    {
        auto idlep = Sjf_PRunning_map[0];
        Sjf_PRunning_map.erase(0);
        Sjf_P_Ready_queue.push(idlep);
        context_switch(idlep, READY);

        auto currprocess = Sjf_P_Ready_queue.top();
        Sjf_P_Ready_queue.pop();
        Sjf_PRunning_map.insert({currprocess->pid, currprocess});
        context_switch(currprocess, RUNNING);
    }
    // If running queue is empty, schedule any process in pq
    else if (Sjf_PRunning_map.size() == 0)
    {
        auto currprocess = Sjf_P_Ready_queue.top();
        Sjf_P_Ready_queue.pop();
        Sjf_PRunning_map.insert({currprocess->pid, currprocess});
