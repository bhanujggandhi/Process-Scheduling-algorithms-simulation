...
priority_queue<PCB *, vector<PCB *>, CompareProcessMLQ> MLQ_Ready_queue;
priority_queue<PCB *, vector<PCB *>, CompareProcessMLQ> MLQ_Device_queue;
...
void scheduleMLQ()
{
    ...
    if (MLQ_Ready_queue.size() >= 1 and MLQRunning_map.size() == 1 and MLQRunning_map.find(0) != MLQRunning_map.end())
    {
        auto idlep = MLQRunning_map[0];
        MLQRunning_map.erase(0);
        MLQ_Ready_queue.push(idlep);
        context_switch(idlep, READY);

        auto currprocess = MLQ_Ready_queue.top();
        MLQ_Ready_queue.pop();
